...
{
    public class SearchQueryContext<TFieldNames> where TFieldNames : IndexFieldNames
    {
		...
		public SearchQueryContext(Database database)
			: this()
		{
			m_database = database;
		}
        ...
        public SearchQueryContext<TFieldNames> Where(Expression<Func<TFieldNames, string>> searchFilterExpression, Guid value)
		{
        ...
        public SearchQueryContext<TFieldNames> Where(Expression<Func<TFieldNames, string>> searchFilterExpression, string value)
		{
			string searchFieldName = ((ConstantExpression) (searchFilterExpression.Body)).Value.ToString();
        ...
        public SearchQueryContext<TFieldNames> LessThan(Expression<Func<TFieldNames, string>> searchFilterExpression, DateTime date)
        {
            string searchFieldName = ((ConstantExpression)(searchFilterExpression.Body)).Value.ToString();
...
//            RangeQuery queryDefinition = new RangeQuery(new Term(searchFieldName, string.Format(SearchIndexer.DateTimeIndexingFormat, date)), new Term(searchFieldName, string.Format(SearchIndexer.DateTimeIndexingFormat, DateTime.MaxValue)), true);
        ...
        public SearchQueryContext<TFieldNames> OrderBy(Expression<Func<TFieldNames, string>> sortFieldExpression, SortOrder sortOrder, ComparisonType comparison)
		{
        ...
        public SearchQueryContext<TFieldNames> OrderByRelevance()
		{
		...
		{
		    IndexSearcher indexSearcher = Index.CreateSearchContext().Searcher;
